function [xmin,fmin,evals,epsilo] = fletcherreeves_show(x,epsil,minx,maxx,stepx,miny,maxy,stepy,lines,f,nablaf,varargin)
% (c) jskl UoM
%
% fletcherreeves_show(x,epsil,minx,maxx,stepx,miny,maxy,stepy,lines,f,nablaf,varargin)
% finds a minimum of a bivariate scalar function by using the Fletcher-Reeves
% conjugate gradient algorithm. Search steps are shown on function contours.
%
...
% Example use:
%
% [xmin,fmin,evals,epsilo] = fletcherreeves_show([-2 2]',1e-9,-2,2,0.05,-2,2,0.05,25,'ex_grad','nabla_ex_grad',2,4,3)
%
...
%
% Try also:
% [xmin,fmin,evals,epsilo] = fletcherreeves_show([0 3]',1e-5,0,3.5,0.05,0,3.1,0.05,40,'ex_cycBS','nabla_ex_cycBS')
% [xmin,fmin,evals,epsilo] = fletcherreeves_show([-2 2]',1e-5,-2,2,0.05,-1,3,0.05,40,'banana','nabla_banana')
%
evals = 0;
...
while norm(grad) >= epsil
   ...
   d = -grad + alpha*d;                 % 2nd deflected direction
