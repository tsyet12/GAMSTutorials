function [xmin,fmin,evals,epsilo] = hookejeeves(x,epsil,f,varargin)
% (c) jskl UoM
%
% hookejeeves(x,epsil,f,varargin)
% finds a minimum of a multivariate scalar function by using
...
% Example use:
%
% [xmin,fmin,evals,epsilo] = hookejeeves([-2 1]',1e-9,'ex_cyc',2,4,3)
%
...
%
% Try also:
% [xmin,fmin,evals,epsilo] = hookejeeves([0 3]',1e-9,'ex_cycBS')
% [xmin,fmin,evals,epsilo] = hookejeeves([-2 2]',1e-9,'banana')
%
evals = 0;
...
while norm(x-z) >= epsil
  ...
  d = x-yy;                        % acceleration step
  yy = x;                          % yy for next iteration
  lambda = fminsearch('flambda',0,OPT,f,x,d,varargin{:});
  x = x + lambda*d;                % moving to the new point
  evals = evals + 1;
end
