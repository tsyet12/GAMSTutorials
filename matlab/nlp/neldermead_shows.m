function [xmin,fmin,evals,epsilo] = neldermead_show(s,epsil,alpha,beta,gama,minx,maxx,stepx,miny,maxy,stepy,lines,f,varargin)
% (c) jskl 2005
%
...
% fmin   = f(xmin)
% evals  = number of iterations (simplex moves)
% epsilo = last distance between the best and the worst points
%
% Example use:
%
% [xmin,fmin,evals,epsilo,ls] = neldermead_show([1 1 2;1 2 1],1e-4,1,0.5,1.5,-2.0,3.0,0.05,-1,3.1,0.05,50,'ex_cyc',2,4,3)
% [xmin,fmin,evals,epsilo,ls] = neldermead_show([-1.5 -1.5 2.5;-0.5 2 0],1e-3,1,0.5,1.5,-2.0,3.0,0.05,-1,3.1,0.05,50,'ex_cyc',2,4,3)
%
...
%
% Try also:
% [xmin,fmin,evals,epsilo,ls] = neldermead_show([0 1 0;2 2 1],1e-5,1,0.5,1.5,0,3.5,0.05,0,3.1,0.05,40,'ex_cycBS')
% [xmin,fmin,evals,epsilo,ls] = neldermead_show([-1 -2 -1;-2 0 -1],1e-5,1,0.5,1.5,-2,2,0.05,-3,2,0.05,40,'banana')
%
evals = 0;
n = length(s) - 1;
hold off;
...
while epsilo >= epsil                   % stop when maximum distance from best smaller than epsil
  s(:,is) = zeros(n,1);                 % "removing" worst point
  xbar = sum(s')'/n;                    % average point
  ...
  hold on;                              % show the next simplex triangle
  plot([s(1,1) s(1,2)],[s(2,1) s(2,2)],style);
  hold on;
